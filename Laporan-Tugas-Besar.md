...
File tubes.cpp merupakan implementasi dari Abstract Data Type (ADT) yang telah didefinisikan pada berkas header. Pada program ini, data pegawai dikelola menggunakan struktur Binary Search Tree (BST) dengan ID pegawai sebagai kunci utama. Setiap pegawai direpresentasikan sebagai sebuah node BST yang berisi informasi ID, nama, jabatan, dan role, serta memiliki pointer ke subtree kiri dan kanan. Fungsi createPegawai digunakan untuk mengalokasikan node pegawai baru, sedangkan fungsi insertBST dan searchBST berperan dalam proses penambahan dan pencarian data pegawai sesuai dengan aturan BST, yaitu data dengan ID lebih kecil ditempatkan di subtree kiri dan data dengan ID lebih besar ditempatkan di subtree kanan.

Traversal BST dilakukan menggunakan metode inorder traversal melalui fungsi inorderBST, sehingga data pegawai dapat ditampilkan dalam urutan ID yang terurut secara menaik. Pada proses penghapusan data pegawai, program menggunakan fungsi hapusPegawai yang menangani seluruh kemungkinan kondisi BST, yaitu node tanpa anak, node dengan satu anak, maupun node dengan dua anak. Untuk kasus dua anak, node pengganti diambil dari nilai minimum pada subtree kanan menggunakan fungsi findMin, sehingga sifat BST tetap terjaga setelah proses penghapusan.

Selain pengelolaan data pegawai, program ini juga mengimplementasikan pengelolaan data absensi menggunakan Multi linked list(satu tingkat). Setiap node pegawai memiliki pointer firstAbsensi yang menunjuk ke linked list absensi miliknya. Setiap node absensi menyimpan informasi tanggal dan jam masuk serta pointer next untuk menghubungkan ke data absensi berikutnya. Penambahan absensi dilakukan melalui fungsi tambahAbsensi dengan mekanisme penyisipan di akhir linked list, sehingga riwayat absensi tersimpan secara kronologis. Data absensi dapat ditampilkan menggunakan fungsi tampilAbsensi, yang melakukan traversal pada linked list absensi milik pegawai tertentu.

Secara konseptual, hubungan antara struktur BST pegawai dan linked list absensi membentuk struktur data bertingkat satu (one-to-many), di mana satu pegawai dapat memiliki banyak data absensi. Struktur ini sering disebut sebagai multi linked list satu tingkat, karena setiap node utama (pegawai) memiliki sub-linked list (absensi). Meskipun belum membentuk multilevel linked list bertingkat lebih dari dua level.

Program ini juga dilengkapi dengan mekanisme penyimpanan data menggunakan berkas CSV. Fungsi loadCSV bertugas membaca data pegawai dan absensi dari file CSV dan membangun kembali struktur BST serta linked list di dalam memori. Sebaliknya, fungsi saveCSV digunakan untuk menyimpan kembali data yang ada di dalam BST dan linked list ke dalam file CSV melalui proses traversal inorder. Dengan adanya mekanisme ini, data tetap bersifat persisten meskipun program dihentikan dan dijalankan kembali.
